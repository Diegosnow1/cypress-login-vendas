...
1 - quando forem homologar qualquer coisas, sempre inserir um "produto fora de estoque", "madeira" e "tinta", porque as 
  ...
  (CT01)Fluxo  Venda completa produto tintometrico
  (CT02)Fluxo  Venda completa produto fora de estoque
  

 * RESULTADO ESPERADO:
 * CASO DE TESTE (CT01): Pedido com produto tintométrico, o sistema deve finalizar o orçamento com a situação de apenas orçando e sem erros.
 * CASO DE TESTE (CT02): Pedido com produto fora de estoque, o sistema deve finalizar o orçamento com a situação de apenas orçando e sem erros.
 * 

  ----------------------------RECOMENDAÇÕES DO DEV--------------------------
2 -sempre criar um pedido, salvar, editar, alterar, salvar novamente
  (CT03)Fluxo  Venda completa/salvar/editar/salvar novamente como orçamento conformado e apenas orçamento.
...
const Itens = (page) => {
  ...
  const FormaPagamento = (page) => {
    ...
    cy.get('[name="formaDePagamento"]', { timeout: 30000 })
      .should('be.visible')
      .then(($select) => {
        if ($select.is(':disabled')) {
          cy.wrap($select).select('A VISTA', { force: true })
          return
        }

        cy.wrap($select).select('A VISTA')
      })
    cy.wait(15000)
    cy.get('[name="tipoDeDocumento"]', { timeout: 30000 })
      .should('be.visible')
      .then(($select) => {
        if ($select.is(':disabled')) {
          cy.wrap($select).select('Dinheiro', { force: true })
          return
        }

        cy.wrap($select).select('Dinheiro')
      })
    cy.wait(15000)
}
